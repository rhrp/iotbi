...
exports.TYPE_DATE_TIME = TYPE_DATE_TIME;
 ...
 * The returned type is acordinf to schema types
...
function getTypeOf(pEntityType,pPropName)
{
   ...
   else if(lTypeObj['anyOf']!=undefined)
   {
      ...
      //debug('Found anyOf '+pEntityType+' /  '+pPropName+' :: AnyOf :: '+JSON.stringify(lTypeObj));
      return undefined;
   }
   else if(lTypeObj['oneOf']!=undefined)
   {
      //Any type is possible!
      //debug('Found oneOf '+pEntityType+' /  '+pPropName+' :: AnyOf :: '+JSON.stringify(lTypeObj));
      return undefined;
   }
...
function getEntitySchema(pEntityType)
{
   ...
   if(lSchemas!=undefined)
   {
       ...
       for(var lUrlSchema in lSchemas)
       {
           ...
           {
              debug('getEntitySchema::Error parsing: '+err.stack);
              return undefined;
           }
       }
...
function parseProperties(pEntityType,pSchemaProperties,pEntitySchema,pSchemaCtxUrl)
{
     ...
     for(var lKey in lParcialSchema)
     {
          ...
          //console.log(lKey+' = '+JSON.stringify(lAttr,null,2))
          pEntitySchema.addAttribute(lKey,lAttr.type,lAttr.itemsType,lAttr.oneOf,lAttr.anyOf,lAttr.object,lAttr.description,lAttr.ref,lAttr.refType,lAttr.format);
     }
}
/**
 * Get the subtree of pDefRefGroup
 */
function getDefinitionsGroup(pSchema,pDefRefGroup)
{
   let lRefPath=pDefRefGroup.split('/');
   let lDefinitionsGroup=undefined;
   for(let lSubPath of lRefPath)
   {
      if(lDefinitionsGroup==undefined)
      {
         lDefinitionsGroup=pSchema.definitions[lSubPath];
      }
      else
      {
         lDefinitionsGroup=lDefinitionsGroup[lSubPath]
      }
   }
   return lDefinitionsGroup;
}
/**
...
{
   lDefinitionsGroup=getDefinitionsGroup(pSchema,pDefRefGroup);
   ...
   if(lDefinitionsGroup==undefined)
   {
       debug('Error getting Definition Group: "'+pDefRefGroup+'"');
       //console.log(JSON.stringify(lDefinitionsGroup,null,2));
   }
   else if(lDefinitionsGroup['type']=='object' && lDefinitionsGroup['properties']!=undefined)
   {
       debug('parseRefDefinitionsGroup "'+pDefRefGroup+'" :: Ok');
       parseProperties(pEntityType,lDefinitionsGroup.properties,pEntitySchema,pSchemaCtxUrl);
   }
   else
   {
       debug('TODO: parseRefDefinitionsGroup "'+pDefRefGroup+'" :: type='+lDefinitionsGroup['type']);
   }
}
/**
...
function toEntityProperty(pEntityType,pPropName,pProperty,pSchemaCtxUrl)
{
    ...
    if(pProperty['format']!=undefined)
    {
        lProp['format']=pProperty['format'];
    }
