...
module.exports = class Table {
   ...
   */
   ...
   selectByAttribs(pAttribs)
   {
      let lCols=this.findColNamesByAttribs(pAttribs);
      lCols.push(schema.COL_ENTITY_ID);
      return this.select(lCols);
   }
   select(pColNames)
   {
       let lSchema={};
       ...
       {
          if(pColNames==undefined || pColNames.includes(lCol))
          {
   ...
   merge(table)
   {
      ...
      //Merge Schema
      for(let lCol in table.getSchema())
      {
          if(this.schema[lCol]==undefined)
          {
             this.schema[lCol]=table.getSchema()[lCol];
          }
       }
      //Merge Rows
  ...
  /**
   * TODO: Implement a data struture to map attribs on cols
   */
   findColNamesByAttribs(pAttribs)
   {
      let lCols=[];
      for(let lColName in this.getSchema())
      {
         ...
         {
           for(let lAttrib of pAttribs)
           {
             if(lColName.startsWith(lAttrib+'_') || lColName==lAttrib)
             {
                lCols.push(lColName);
             }
           }
