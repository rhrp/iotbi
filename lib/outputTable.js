/*!
 * Utilities related to tables outputs
...
function isValidFormat(pFormat)
{
   if(pFormat==undefined)
   {
        return false;
   }
   else if(['json','csv','parquet'].includes(pFormat))
   {
        return true;
   }
   else
...
/**
 * Convert a table model to CSV
 */
function toCSV(pTableModel)
{
   debug('CSV:: Rows: '+pTableModel.countRows()+'  Cols: '+pTableModel.countCols())
   var lCsv='';
   if(pTableModel.countRows()==0)
   {
      var n=0;
      for(var h in pTableModel.getSchema())
      {
          if(n==0)
              lCsv='"'+h+"'";
          else
              lCsv=(lCsv+DELIMITER+'"'+h+'"');
          n=n+1;
      }
   }
   else
   {
      ...
      lCsv = parser.parse(pTableModel.getRows());
   }
   //console.log('----------------------');
   //console.log(lCsv);
   //console.log(JSON.stringify(pTableModel,null,2));
   //console.log('----------------------');
   return lCsv;
}
...
{
   var lTable=pTableAndSchema.getRows();
   var lSchema=pTableAndSchema.getSchema();
   ...
   try
   {
        ...
        {
           var lOut=toCSV(pTableAndSchema);
...
function __deprecated_sendAsCSV(pTableValues) 
{
   const opts = {delimiter:DELIMITER};
   const parser = new Parser(opts);
...
/*
      ...
      {
         debug('Col: '+key+' inferSchema: Ignore Object ',key);
      }
*/
      else
      {
         debug('Col: '+key+' inferSchema: Ignore ',key+' Type:'+lType+'  TypeOf:'+(typeof r[key])+'   Value: '+JSON.stringify(r[key]));
      }
      n=n+1;
  }
  //debug('Schema with '+n+' cols:\n'+JSON.stringify(lSchemaJson,null,2));
  debug('Schema with '+n+' cols.  HelperSchema: '+Object.keys(pSchemaHelper).length+' cols');
  return lSchemaJson;
}
function getRowJson(pSchema,pSchemaParquetJson,pTableValues,pRowId)
{
  //debug('Schema: '+JSON.stringify(pSchema))
  var r=pTableValues[pRowId];
  var lRowJson={};
  // Make the row based on pSchemaParquetJson
  for (var key in pSchemaParquetJson)
  {
      ...
      if(lType==schema.DATETIME)
      {
        if(r[key]==undefined)
        {
            //debug('Correcting DateTime type of col '+key+' at row '+pRowId+'  value='+r[key]);
            lValue=undefined;
        }
        else
        {
      ...
      else if(lType==schema.STRING && typeof r[key] != 'string')
      {
        lValue=''+(r[key]!=undefined?r[key]:'');
        //debug('Correcting String type of col '+key+' at row '+pRowId);
      }
      else
      {
        //debug('No correction is nead fo col '+key+' at row '+pRowId);
        lValue=r[key];
      }
      lRowJson[key]=lValue;
  }
...
function createPromiseSaveInCacheAsParquet(pFilename,pTableValues,pEntityName,pSchema)
{
  ...
  return new Promise(async function(resolve, reject) {
      ...
      {
          var lRow=getRowJson(pSchema,lSchemaParquetJson,pTableValues,i);
...
{
   //debug('Table as CSV: '+JSON.stringify(pTableValues));
   if(pTableValues.length>0)
   {
     const opts = {delimiter:DELIMITER};
     const parser = new Parser(opts);
     var lCsv = parser.parse(pTableValues);
   ...
   else
   {
       ...
       var n=0;
       //debug('Schema: '+JSON.stringify(pSchema));
       for(var h in pSchema)
       {
         lCsv=lCsv+(n==0?'':DELIMITER)+'"'+h+'"';
         n=n+1;
       }
   }
   debug('Output: '+lCsv);
