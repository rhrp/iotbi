...
module.exports = class Rank {
   ...
   */
   async makeEmbeddings()
   {
      ...
      debug(`Generate embeddings for ${lSentences.length} sentences`);
   ...
   */
   async similarityText(pColName,pText)
   {
       ...
       /**
       if(lSentences.includes('Piscina') || lSentences.includes('piscina') || lSentences.includes('Piscinas') || lSentences.includes('piscinas'))
       {
         if(lScore.hasScore() || true)
         {
   ...
   */
   async getIndexSortedBySimilarity()
   {
    ...
    async sortByRelevance(topk,threshold,maxRowsUnScored) 
    {
       ...
       let lTopk=topk || this.table.countRows();
       let lScoresTable=[];
       ...
       {
          let lOk=false;
          if(lTopk!=undefined && lScore.score.similarity>lThresholdScore && lAcceptedTopK<lTopk)
          {
             //debug('add '+lScore.key+'   score='+lScore.score.similarity);
             lAcceptedTopK=lAcceptedTopK+1;
             lOk=true;
          }
          else if(lTopk!=undefined && lScore.score.similarity>lThresholdScore)
          {
             lExcludedOutTopK=lExcludedOutTopK+1;
          }
          else if(lTopk!=undefined && lAcceptedLowScore<maxRowsUnScored)
          {
       ...
       debug('Initial table '+this.table.countRows()+' rows, lTopk='+lTopk+' maxRowsUnScored='+maxRowsUnScored+'  sorted/filtred table '+lSortedTable.countRows()+' rows (topk '+lAcceptedTopK+' and low score '+lAcceptedLowScore+'). IgnoredLowScore='+lIgnoredLowScore+' ExcludedOutTopK='+lExcludedOutTopK);
       let lMsg;
