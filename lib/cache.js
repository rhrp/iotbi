 ...
 */
exports.genCurrentWebHDFSCacheKey = function(pFiwareService,pEntityType,pJoinAttrib,pJoinEntityType,pDataset)
{
    var lDataset=(pDataset==undefined?'undefined_dataset':pDataset);
    var lCachePlainKey='dataset_'+lDataset+'_'+pFiwareService+'_'+pEntityType+(pJoinAttrib!=undefined?'_'+pJoinAttrib:'')+(pJoinEntityType!=undefined?'_'+pJoinEntityType:'');
    debug('Cache Key Plain of a current dataset: '+lCachePlainKey);
    return genHash(lCachePlainKey);
}
/**
 * Generates the key (plain) of a Schema webhdfs Request in the Cache
 */
exports.genSchemaWebHDFSCacheKey = function(pFiwareService,pEntityType)
{
    var lCachePlainKey='schema_'+pFiwareService+'_'+pEntityType;
    debug('Cache Key Plain of a Schema: '+lCachePlainKey);
    return genHash(lCachePlainKey);
}
