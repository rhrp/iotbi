...
class EntitySchema
{
  ...
  /**
   * Find the items types acording to NGSI types and formats
   */ 
  findItemTypes(pAttribName,pXxxOf)
  {
      if(pXxxOf==undefined)
      {
         return undefiend;
      ...
      for(var t of pXxxOf)
      {
          if(t['type']=="string" && t['format']=="uri")
          {
               lOut.push(sdm.TYPE_ENTITY_IDENTIFIER);
          }
          else if(t['type']=="string" && t['format']=="date-time")
          {
               //TODO: test this case
               return sdm.TYPE_DATE_TIME;
          }
          else
          {
               lOut.push(t['type']);
  ...
  addAttribute(pName,pType,pItemType,pOneOf,pAnyOf,pObject,pDescription,pRef,pRefType,pFormat) 
  {
      ...
      if(pOneOf!=undefined)
      {
        ...
        this.schema[pName].itemTypes = this.findItemTypes(pName,pOneOf);
      }
      if(pAnyOf!=undefined)
      {
        this.schema[pName].anyOf = pAnyOf;
        this.schema[pName].itemTypes = this.findItemTypes(pName,pAnyOf);
      }
  ...
  getRelationShipFields()
  {
     ...
     for(var lAttribName in this.schema)
     {
       ...
       //debug('getRelationShipFields::'+lAttribName+'   type='+lSchemaAttr.type+'   itemType='+lSchemaAttr.itemType+'    itemTypes='+lSchemaAttr.itemTypes)
       //TODO: use isTypeRelationshipByName(pAttribName)
       if(lSchemaAttr.type==sdm.TYPE_ENTITY_IDENTIFIER 
       || (lSchemaAttr.itemType!=undefined && lSchemaAttr.itemType==sdm.TYPE_ENTITY_IDENTIFIER)
       || lSchemaAttr.itemTypes!=undefined && lSchemaAttr.itemTypes.includes(sdm.TYPE_ENTITY_IDENTIFIER))
       {
  ...
  */
  ...
  isTypeRelationshipByName(pAttribName)
  {
    var lField=this.getFieldByName(pAttribName);
    if(lField.type==sdm.TYPE_ENTITY_IDENTIFIER 
       || (lField.itemType!=undefined && lField.itemType==sdm.TYPE_ENTITY_IDENTIFIER)
       || lField.itemTypes!=undefined && lField.itemTypes.includes(sdm.TYPE_ENTITY_IDENTIFIER))
       {
          return pAttribName!=EntitySchema.NGSI_CORE_ID;
       }
    return false
  }
