...
TA_LIMIT[BROKER_STELLIO.toLowerCase()]=1000;
...
exports.getBrokerNGSIVersion = function(pFiwareService) {
   //debug('getBrokerNGSIVersion('+pFiwareService+')='+JSON.stringify(config.broker.servers[pFiwareService]));
...
exports.allowMcpToUser = function (pMcpUserName)
{
    if(pMcpUserName==undefined)
    {
        debug('Undefined mcp user');
        return false;
    }
    else if(config.security.appKeys[pMcpUserName]==undefined)
    {
        debug('Invalid mcp user: '+pMcpUserName);
        return false;
    }
    else
    {
        return config.security.appKeys[pMcpUserName].allowMcp;
    }
}
/**
...
/**
 * Returns a list of allowed fiware services to the appKey
 */
exports.allowedFiwareServicesToKey = function(pAppKey)
{
    if(pAppKey==undefined)
    {
        debug('Undefined pAppKey');
    ...
    {
        let lFiwareServices=[];
        for(let s of config.security.appKeys[pAppKey].scopes)
        {
           if(this.isConfigOk(s))
           {
              lFiwareServices.push(s);
           }
        }
        return lFiwareServices;
    }
}
...
// The geoquery is done after loading from broker. In my Stellio there was a problem with geo queries
const LOCAL_GEOQUERY=false;
...
/**
 * Get the transformer to be used for sentence similarity
 */
exports.getTransformerSentenceSimilarity = function(pAppKey=undefined)
{
   let path='./topk/transformer/'+config.transformer.similariy.default+'.js';
   let Transformer=require(path);
   let lTransformer=new Transformer();
   debug('Similarity based on: '+lTransformer.about());
   return lTransformer;
}
