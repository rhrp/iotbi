   ...
   return new Promise(function(resolve, reject)
   {
     ...
     request(lOptions, function (error, response, pBody) {
        try
        {
                ...
                console.log('['+pFiwareService+']['+pFiwareServicePath+'] Invoke:'+lUrl);
...
{
   console.log('Start the join of Main table with '+utils.describeTable(pMainTable)+' rows and a join table containing '+utils.describeTable(pJoinTable)+' rows ...');
   ...
   console.log('Finish the join getting a table containing '+utils.describeTable(pMainTable)+' rows ...');
   return pMainTable;
}
	 ...
	 //console.log('Row #'+i+'  '+lKeyAttr);
       ...
       for(var lTimeKey in lTmp)
       {
          ...
          for(var lPropKey in lTmp[lTimeKey])
          {
             ...
             {
                 console.log('Warning:   Ignored '+lEntity.id+'   '+lTimeKey+'  '+lPropKey+' '+JSON.stringify(lValueTime));
             }
          }
          lTable.push(lRow);
       }
...
function createPromiseQueryEntities(pFiwareService,pFiwareServicePath,pEntityType,pEntityId)
{
   ...
   var lOptions = {"tenant":pFiwareService,
                   ...
                   //"@context":["http://context/json-context.jsonld","https://uri.etsi.org/ngsi-ld/v1/ngsi-ld-core-context-v1.7.jsonld"],
                   "limit":1000
                  };
   ...
   //console.log('Broker URL: '+lBrokerURL);
   //console.log('Options: '+JSON.stringify(lOptions,null,2));
