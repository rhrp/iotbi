...
//Deprecated use configsys.getBrokerPageSize()
 ...
 */
function listEntities(pEndpoint,pObjQuery,pFiwareService,pFiwareServicePath,pExtended,pTableNameJoin,pJoinAttribute,pRequestId)
{
   // Start the loading by the context data and then the data
   var lEntityType=pObjQuery['type'];
   return ngsildcontext.createPromisesloadNgsiContext(utils.toArray(pFiwareService,undefined),utils.toArray(lEntityType,pTableNameJoin)).then(function (lNgsiContextData) {
                 setNgsiContextData(lNgsiContextData);
                 return loadMainTable(pEndpoint,pObjQuery,pFiwareService,pFiwareServicePath,pExtended,pTableNameJoin,pJoinAttribute,pRequestId);
   }).catch(function(err) {
 ...
 */
function loadMainTable(pEndpoint,pObjQuery,pFiwareService,pFiwareServicePath,pExtended,pTableNameJoin,pJoinAttribute,pRequestId)
{
    var lLimit=configsys.getBrokerPageSize(pFiwareService);
    return loadMainTablePage(pEndpoint,pObjQuery,pFiwareService,pFiwareServicePath,pExtended,pTableNameJoin,pJoinAttribute,0,lLimit,[],pRequestId,1);
}
function loadMainTablePage(pEndpoint,pObjQuery,pFiwareService,pFiwareServicePath,pExtended,pTableNameJoin,pJoinAttribute,pOffset,pLimit,pEntitiesAcumulator,pRequestId,pPageId)
{
   ...
   var lUrl=pEndpoint+'/entities?type='+pObjQuery.type
	   ...
	   +(pObjQuery.idPattern!=null?'&idPattern='+pObjQuery.idPattern:'')
           +(pObjQuery.q!=null?'&q='+pObjQuery.q:'')
           +(pObjQuery.georel!=null?'&georel='+pObjQuery.georel:'')
           +(pObjQuery.geometry!=null?'&geometry='+pObjQuery.geometry:'')
           +(pObjQuery.coordinates!=null?'&coordinates='+pObjQuery.coordinates:'')
           +(pObjQuery.geoproperty!=null?'&geoproperty='+pObjQuery.geoproperty:'');
   //debug('URL: '+lUrl);
...
                   debug('error:', error);
                ...
                {
                   //debug('loadMainTablePage::statusCode:', response && response.statusCode);
                   if(response.statusCode<200 || response.statusCode>299)
                   {
                        debug('['+pFiwareService+']['+pFiwareServicePath+'] Invoke:'+lUrl);
                        debug('Error Body: '+JSON.stringify(response['body'],null,2));
                        return reject({'step':'Loading Main Table','FiwareService':pFiwareService,'Code':response.statusCode});
                   }
                   else
                   {
                        var lEntitiesPage=JSON.parse(pBody);
                        if(lEntitiesPage.length>0)
                        {
                            pEntitiesAcumulator=pEntitiesAcumulator.concat(lEntitiesPage);
                        }
                        ...
                        //debug('Page:'+lEntitiesPage.length+' Entities='+pEntitiesAcumulator.length+'  offset='+pOffset+'   limit='+lLimit);
                        if(lEntitiesPage.length==pLimit)
                        {
                            // Read next page
                            var lOffset=pOffset+pLimit;
                            return resolve(loadMainTablePage(pEndpoint,pObjQuery,pFiwareService,pFiwareServicePath,pExtended,pTableNameJoin,pJoinAttribute,lOffset,pLimit,pEntitiesAcumulator,pRequestId,pPageId+1));
                        }
                        else
                        {
                            // Last page
                            var lTable=entitiesToTable(pEntitiesAcumulator,pExtended);
                            let lCountTableRows=lTable.countRows();
                            //debug('TableRows: '+lCountTableRows);
                            //TODO:  measure time in case of join
                            ...
                            {
                               debug('No JOIN')
                               //debug(JSON.stringify(lTable,null,2));
...
function entitiesToTable(pEntities,pExtended)
{
  ...
  var lTable=new TableModel('NGSI-LD-V1-no-name',lTable,lSchema);
  return lTable; 
}
  ...
  return new Promise(function(resolve, reject)
  {
     ...
     request(lOptions, function (error, response, pBody) {
        try
        {
                ...
                if(error!=null)
                {
                 console.error('loadJoinTable::error:', error);
                   return reject({'step':'Loading Join Table '+pEntityName,'description':error});
                }
                else
                {
                 debug('loadJoinTable::statusCode:', response && response.statusCode);
                   if(response.statusCode<200 || response.statusCode>299)
                   {
...
{
   var lMainTable=pMainTablePlusSchema.getRows();
   var lJoinTable=pJoinTablePlusSchema.getRows();
   var lMainSchema=pMainTablePlusSchema.getSchema();
   var lJoinSchema=pJoinTablePlusSchema.getSchema();
   ...
   var lTable=new TableModel('NGSI-LD-V1-join-no-name',lMainTable,lMainSchema);
   return lTable; 
}
...
{
var zzz = require('ngsijs');
   var lBrokerURL=configsys.getBrokerURL(pFiwareService);
   var lTenant=configsys.getBrokerTenant(pFiwareService);
   var lConnection = new zzz.Connection(lBrokerURL);
   var lOptions = {
                   "tenant":lTenant,
                  };
   debug('Broker URL: '+lBrokerURL);
...
function createPromiseGetType(pFiwareService,pEntityType)
{
   ...
   debug('Broker URL: '+lBrokerURL);
   debug('Options: '+JSON.stringify(lOptions));
   debug('Invoke lConnection.ld.getType()');
   //Load the type's data
   return lConnection.ld.getType(lOptions);
}
function createPromiseQueryEntitiesAttributes(pFiwareService,pEntityType,pAttribName,pOffset,pLimit)
{
   var lBrokerURL=configsys.getBrokerURL(pFiwareService);
   var lTenant=configsys.getBrokerTenant(pFiwareService);
   var lContext=getNgsiLdEntityContext(pFiwareService,pEntityType);
...
function listGeoMetadataEntities(pFiwareService,pFiwareServicePath,pEntityType,pRadius)
{
    ...
    var lLimit=configsys.getBrokerPageSize(pFiwareService);
    return listGeoMetadataEntitiesPage(pFiwareService,pFiwareServicePath,pEntityType,pRadius,0,lLimit,[]);
}
function listGeoMetadataEntitiesPage(pFiwareService,pFiwareServicePath,pEntityType,pRadius,pOffset,pLimit,pPointsAcumulator)
{
   debug('listGeoMetadataEntitiesPage('+pFiwareService+','+pFiwareServicePath+','+pEntityType+','+pRadius+') offset='+pOffset);
   var lNgsiCoreGeospatial=['location','observationSpace','operationSpace']
   return createPromiseQueryEntitiesAttributes(pFiwareService,pEntityType,lNgsiCoreGeospatial,pOffset,pLimit)
      .then((lEntities) => { 
                             return new Promise(function(resolve, reject)
                                        {
                                          //debug('Entities: \n'+JSON.stringify(lEntities.results,null,2));
                                          //var lPoints=[];
                                          var n=0;
                                          for (var lEntity of lEntities.results)
                                          {
                                             //debug('Entity: \n'+JSON.stringify(lEntity,null,2));
                                             var lRowAndSchema=entityToRow(lEntity,false);
                                             for(lAttrib of lNgsiCoreGeospatial)
                                             {
                                                var lAttribLongitude=lAttrib+'_coordinates_lon';
                                                var lAttribLatitude=lAttrib+'_coordinates_lat';
                                                if(lRowAndSchema[0][lAttribLongitude]!=undefined && lRowAndSchema[0][lAttribLatitude]!=undefined)
                                                {
                                                   //debug('GeoSpatial data in attribute \"'+lAttrib+'\" in entity '+pEntityType);
                                                   var lPoint=[
                                                           lRowAndSchema[0][lAttribLongitude],
                                                           lRowAndSchema[0][lAttribLatitude]
                                                              ];
                                                   //lPoints.push(lPoint);
                                                   pPointsAcumulator.push(lPoint);
                                                   n=n+1;
                                                }
                                             }
                                          }
                                          ...
                                          {
                                             var lOffset=pOffset+pLimit;
                                             resolve(listGeoMetadataEntitiesPage(pFiwareService,pFiwareServicePath,pEntityType,pRadius,lOffset,pLimit,pPointsAcumulator));
                                          }
                                          else
                                          {
                                             var lTable=utilsGeo.toTableGetMetadata(pFiwareService,pEntityType,pRadius,pPointsAcumulator);
                                             resolve(lTable);
                                          }
                                        });
                           })
