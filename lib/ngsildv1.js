...
function listEntities(pEndpoint,pObjQuery,pFiwareService,pFiwareServicePath,pExtended,pEntityType,pTableNameJoin,pJoinAttribute)
{
   ...
   return ngsildcontext.createPromisesloadNgsiContext(utils.toArray(pFiwareService,undefined),utils.toArray(pEntityType,pTableNameJoin)).then(function (lNgsiContextData) {
...
                   debug('statusCode:', response && response.statusCode);
			...
			//XXXXXXXXXXXXX var lTable=entitiesToTable(lEntitiesPage,pExtended);
 ...
 */
function listGeoMetadataEntities(pFiwareService,pFiwareServicePath,pEntityType)
{
    debug('listGeoMetadataEntities('+pFiwareService+','+pFiwareServicePath+','+pEntityType+')');
    var lLimit=configsys.getBrokerPageSize(pFiwareService);
    return listGeoMetadataEntitiesPage(pFiwareService,pFiwareServicePath,pEntityType,0,lLimit,[]);
}
function listGeoMetadataEntitiesPage(pFiwareService,pFiwareServicePath,pEntityType,pOffset,pLimit,pPointsAcumulator)
{
   debug('listGeoMetadataEntitiesPage('+pFiwareService+','+pFiwareServicePath+','+pEntityType+')  pffset='+pOffset);
                             ...
                             return new Promise(function(resolve, reject)
                                        {
                                          ...
                                          {
                                             var lOffset=pOffset+pLimit;
                                             resolve(listGeoMetadataEntitiesPage(pFiwareService,pFiwareServicePath,pEntityType,lOffset,pLimit,pPointsAcumulator));
                                          }
                                          else
                                          {
                                             resolve(utilsGeo.toTableGetMetadata(pFiwareService,pEntityType,pPointsAcumulator));
                                          }
                                        });
                           })
      .catch((err)      => { 
