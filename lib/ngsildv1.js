...
function listEntities(pEndpoint,pObjQuery,pFiwareService,pFiwareServicePath,pExtended,pEntityType,pTableNameJoin)
{
   ...
   return ngsildcontext.createPromisesloadNgsiContext(pFiwareService).then(function (lNgsiContextData) {
...
function getInvokeFiwareHeaders(pFiwareService,pFiwareServicePath,pEntityType) 
{
   ...
   const headers= {};
   headers['NGSILD-Tenant']=apiConfig.getBrokerTenant(pFiwareService);
...
function needLoading(pUrl)
{
  ...
  else if(gNgsiContext[pUrl]==undefined)
  {  
...
//   gNgsiContext={};
   ...
   return Promise.all(lPromises).then(function(values) {
        debug('Promise.all:'+values.length);
...
function createPromiseQueryEntities(pFiwareService,pFiwareServicePath,pEntityType,pEntityId)
{
   ...
   var lOptions = {"tenant":apiConfig.getBrokerTenant(pFiwareService),
                   "@context":lContext,
