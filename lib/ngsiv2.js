...
exports.listGeoMetadataEntities = listGeoMetadataEntities;
...
function entitiesToTable(pEntities,pExtended)
{
   ...
   //debug('Total Rows: '+lTable.length)
   //debug('Schema: '+JSON.stringify(lSchema,null,2));
   //return [lTable,lSchema];
   var lTable=new TableModel('NGSI-v2-table-no-name',lTable,lSchema);
   return lTable; 
}
/**
...
function entityToRow(pEntity,pExtended)
{
	...
	if(lDataField=='id')
	{
            //debug('Entity Id: '+pEntity[lDataField]);
        }
        else if(lDataField=='type')
        {
            //debug('Entity Type: '+pEntity[lDataField]);
        }
        else
        {
             let lEntityAttrib=pEntity[lDataField];
             let lTypeOfValue=typeof lEntityAttrib.value;
             //debug('Object Id '+pEntity.id+'  :: Prop='+lDataField+'  TypeOf: '+lTypeOfValue+' EntityType: '+lEntityAttrib.type+' ::  '+JSON.stringify(lEntityAttrib.value));
             //if(('\"'+lEntityAttrib.value+'\"')==JSON.stringify(lEntityAttrib.value)
             //|| (''+lEntityAttrib.value)==JSON.stringify(lEntityAttrib.value))
             if(lTypeOfValue === 'string' || lTypeOfValue === 'number')
	     {
 		  ...
 		  lReg[lColName]=lValue;
                  //debug('Col: '+lDataField+'   Type='+lType+' ('+(typeof lEntityAttrib.value)+') Value='+lValue+'  Original value='+lEntityAttrib.value);
             }
             else if(lTypeOfValue === 'object' && lEntityAttrib.type=='geo:json' && lEntityAttrib.value['type']=='Point')
             {
             ...
             else if(lEntityAttrib.value!=undefined && lEntityAttrib.value['type']=='MultiLineString')
             {
             ...
             else if(lEntityAttrib.value!=undefined && Array.isArray(lEntityAttrib.value))
             {
                 //An Array of objects
                 flattenValue(lReg,lSchema,lColName,lEntityAttrib.value,0);
             }
             else if(lEntityAttrib.value!=undefined && lTypeOfValue === 'object')
             {
                 // An Object
		 //debug('Object Id '+pEntity.id+'  :: Prop='+lDataField+'  TypeOf: '+lTypeOfValue+' EntityType: '+lEntityAttrib.type+'  ::  Expand value <'+JSON.stringify(lEntityAttrib.value)+'>');
                 flattenValue(lReg,lSchema,lColName,lEntityAttrib.value,0);
             }
             else
             {
                  debug('Object Id '+pEntity.id+'  :: Prop='+lDataField+'  TypeOf: '+lTypeOfValue+' ::  Expand value <'+JSON.stringify(lEntityAttrib.value)+'>');
                  for(var lValueField in lEntityAttrib.value)
                  {
                        ...
                        debug(' *  TypeString='+lTypeString+'  Type='+lType+'   Value='+lValue);
                  }
             }
             //if(pEntity[lDataField].type=='geo:json')
             if(lTypeOfValue === 'object' && lEntityAttrib.type=='geo:json')
             {
...
function flattenValue(pReg,pSchema,pColName,pObject,pLevel)
{
   let lLevel=pLevel+1;
   let lTypeOfObject;
   if(pObject==undefined)
   {
       debug('  ['+pLevel+'] '+pColName+' Null Object');
   }
   else if(Array.isArray(pObject))
   {
       for(var lIdx in pObject)
       {
          let lValue=pObject[lIdx];
          let lColNameValue=pColName+'_value_'+lIdx;
          lTypeOfValue=flattenValue(pReg,pSchema,lColNameValue,lValue,lLevel);
       }
       pReg[pColName+'_type']='Array['+lTypeOfValue+']';
       pSchema[pColName+'_type']=schema.STRING;
       lTypeOfObject='array';  
   }
   else
   {
       lTypeOfObject=typeof pObject;
       if(lTypeOfObject === 'string')
       {
          let lType=schema.inferType(pObject);
          pReg[pColName]=pObject;
          pSchema[pColName]=lType;
       }
       else if(lTypeOfObject === 'object')
       {
          let lKeys = Object.keys(pObject);
          for(let lKey of lKeys)
          {
            let lProp=pObject[lKey];
            let lTypeOfProp=typeof lProp;
            flattenValue(pReg,pSchema,pColName+'_'+lKey,lProp,lLevel);
          }
       } 
       else
       {
          debug('flattenValue: '+pColName+'  invalid type '+lTypeOfObject);
       }
   }
   //debug('  ['+pLevel+'] '+pColName+' typeOf:'+lTypeOfObject+'  '+JSON.stringify(pObject));
   return lTypeOfObject;
}
...
function createPromiseListEntities(pFiwareService,pFiwareServicePath,pExtended,pEntityType,pObjQuery,pOffset,pLimit)
{
   ...
   if(pObjQuery!=undefined)
   {
     ...
     if(pObjQuery.georel!=undefined && pObjQuery.geometry!=undefined && pObjQuery.coords!=undefined)
     {
        //https://fiware-orion.readthedocs.io/en/master/orion-api.html
        lOptions['georel']=pObjQuery.georel;
        lOptions['geometry']=pObjQuery.geometry;
        lOptions['coords']=pObjQuery.coords;
        // Remove option count 
        if(lOptions['count']!=undefined)
        {
           debug('Option count was removed!  Aparentely, is not compatible with geoqueries');
           delete lOptions.count;
        }     
     }
     else
     {
        debug('No geoquery:: Georel='+pObjQuery.georel+' geometry='+pObjQuery.geometry+'   coods='+pObjQuery.coords+'  geoproperty='+pObjQuery.geoproperty);
     }
     if(pObjQuery.id!=undefined)
     {
         lOptions['id']=pObjQuery.id;
     }
   }
   debug('Broker URL: '+lBrokerURL);
   debug('Options: '+JSON.stringify(lOptions,null,2));
   //Load entities
 ...
 */
function listEntities(pEndpoint,pObjQuery,pFiwareService,pFiwareServicePath,pExtended,pEntityType,pRequestId)
{
   debug('listEntities('+pEndpoint+','+JSON.stringify(pObjQuery)+','+pFiwareService+','+pFiwareServicePath+','+pExtended+','+pEntityType+')');
   return listEntitiesPagined([],pFiwareService,pFiwareServicePath,pExtended,pEntityType,pObjQuery,0,500,pRequestId,1)
}
/**
 * The listEntities to be recursive called 
 */
function listEntitiesPagined(pEntities,pFiwareService,pFiwareServicePath,pExtended,pEntityType,pObjQuery,pOffset,pLimit,pRequestId,pPageId)
{
   perfanalyser.mark(pRequestId,perfanalyser.OP_LP,'startPage',pFiwareService,pPageId);
   return createPromiseListEntities(pFiwareService,pFiwareServicePath,pExtended,pEntityType,pObjQuery,pOffset,pLimit)
          .then((result) => {
                               perfanalyser.mark(pRequestId,perfanalyser.OP_LP,'endPage',pFiwareService,pPageId);
                               //debug('Result:'+JSON.stringify(result,null,2));
                               var lTotal=result.count;
                               var lCorrelator=result.correlator;
                               var lEntitiesPage=result.results;
                               var lAllEntities=pEntities.concat(lEntitiesPage);
                               debug('FiwareService: '+pFiwareService+'  EntityType: '+pEntityType+'  Correlator:'+lCorrelator+'    Count: '+lTotal+'   Rows:'+result.results.length);
                               if(lTotal==undefined)
                               {
                                    lTotal=result.results.length;
                                    debug('Count is not available. Using result.results.length='+lTotal);
                               }
                               perfanalyser.mark(pRequestId,perfanalyser.OP_LP,perfanalyser.EVENT_PAGE_COLLECT,pFiwareService,pPageId,lEntitiesPage.length);
                               //if(lTotal<=pLimit)
                               //{
                               ...
                               //               //debug('TableAndSchema: '+JSON.stringify(lTable));
                               //               resolve(lTable);
                               //           });
                               //}
                               //else 
                               if(lEntitiesPage.length<pLimit)
                               {
                                   let lCountTableRows=lAllEntities.length;
                                   debug(`Last page. Records: ${lEntitiesPage.length} of ${lTotal} (${lCountTableRows})`);
                                   perfanalyser.mark(pRequestId,perfanalyser.OP_LP,perfanalyser.EVENT_END_BROKER_CALL,pFiwareService,pPageId,lCountTableRows);
                               ...
                               else
                               {
                                   ...
                                   debug('Next page  Total='+lTotal+'  Limit='+pLimit+'   Offset='+lOffset+'   Loaded='+lEntitiesPage.length+' Previous loaded: '+lAllEntities.length); 
                                   return listEntitiesPagined(lAllEntities,pFiwareService,pFiwareServicePath,pExtended,pEntityType,pObjQuery,lOffset,pLimit,pRequestId,pPageId+1)
                               }
                            })
          .catch((err) => {
                              debug('Error loading current offset '+pOffset+'/'+pLimit+' of '+pEntityType+' in '+pFiwareService+': '+JSON.stringify(err));
                              return new Promise(function(resolve, reject)
                                         {
                                             reject(err);
                                         });
...
/**
 * Get geo metadata about an Entity Type
 * TODO: Implement for ['location','observationSpace','operationSpace'] 
 */
function listGeoMetadataEntities(pFiwareService,pFiwareServicePath,pEntityType,pRadius)
{
   debug('listGeoMetadataEntities('+pFiwareService+','+pFiwareServicePath+','+pEntityType+','+pRadius+')');
   var lObjQuery=undefined;
   return listEntities(undefined,lObjQuery,pFiwareService,pFiwareServicePath,false,pEntityType)
      .then((lTableAndSchema) => { 
                             //debug("listGeoMetadataEntities lTableAndSchema: \n"+JSON.stringify(lTableAndSchema,null,2))
                             return new Promise(function(resolve, reject)
                                        {
                                          var lPoints=[];
                                          for (var lRow of lTableAndSchema.getRows())
                                          {
                                             //debug('EntityRow: \n'+JSON.stringify(lRow,null,2));
                                             var lPoint=[
                                                           lRow['location_coordinates_lon'],
                                                           lRow['location_coordinates_lat']
                                                        ];
                                             lPoints.push(lPoint);
                                          }
                                          resolve(utilsGeo.toTableGetMetadata(pFiwareService,pEntityType,pRadius,lPoints));
                                        });
                           })
      .catch((err)      => { 
                               debug('listGeoMetadataEntities Error: ', err); 
                           });
}
