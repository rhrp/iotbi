 ...
 */
async function createPromisesloadNgsiContext(pFiwareServices,pEntityTypes)
{
//TODO: caso nao haja definicao para uma da Entities, o carregamento baralha-se 
   ...
   for(lFiwareService of pFiwareServices)
   {
      debug('Create promises of loading "'+lFiwareService+'" contexts');
      if(!configsys.isConfigOk(lFiwareService))
      {
         debug('FiwareService "'+lFiwareService+'" have an invalid config');
      }
      else if(configsys.isVersionLDV1(lFiwareService))
      {
        var lUrlsNgsiLdOfThisService=getNgsiLdContexts(lFiwareService);
        for(var lUrl of lUrlsNgsiLdOfThisService)
        {
          lUrlsNgsiLd.push(lUrl);
          if(needLoading(lUrl))
          {
      ...
      {
          debug('FiwareService "'+lFiwareService+'" does not have contexts');
      }
   }
   var lUrlsCommonSchemas=[];
   // Common Schemas for this Service
   for(lFiwareService of pFiwareServices)
   {
      debug('Create promises of loading "'+lFiwareService+'" Schemas');
      if(!configsys.isConfigOk(lFiwareService))
      {
         debug('FiwareService "'+lFiwareService+'" have an invalid config');
      }
      ...
      {
         var lUrlsCommonSchemasOfThisService=configsys.getBrokerCommonsSchemas(lFiwareService); 
         debug(JSON.stringify(lUrlsCommonSchemasOfThisService));
         for(var lUrl of lUrlsCommonSchemasOfThisService)
         {
            lUrlsCommonSchemas.push(lUrl);
      ...
      {
          debug('FiwareService "'+lFiwareService+'" does not support schemas');
      }
   }
   // For this entity
   ...
   return Promise.all(lPromises).then(function(values) {
        ...
        for(var i=0;i<values.length;i++)
        {
            ...
            else if(lUrlsCommonSchemas.includes(lUrl))
            {
              ...
              //debug('Load Common Schema of '+pFiwareServices+': '+lUrl);
            }
