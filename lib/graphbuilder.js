...
function getGraph(pName)
{
   ...
   lNode=lGraph.addNode(GraphModel.NGSI_RELATIONSHIP,GraphModel.NGSI_TYPE);
   lNode.addProperty('description','A relationhip to another NGSI entity')
   addEntities(lGraph);
   return lGraph;
}
...
function addEntityProperties(pGraph,pNgsiEntityType)
{
   ...
   for(lPropName of lEntitySchema.getAllNameFields())
   {
      ...
      if(lEntitySchema.isTypeRelationshipByName(lPropName))
      {
        lNode=pGraph.addNode(lPropId,GraphModel.NGSI_RELATIONSHIP);
        lNode.addProperty('name',lPropName);
        lNode.addProperty('description',lProp.description);
        lEdge=pGraph.addRelationship(pNgsiEntityType,lPropId,'has relationship');
      }
      else
      {
        ...
        lNode.addProperty('name',lPropName);
        lNode.addProperty('description',lProp.description);
        lEdge=pGraph.addRelationship(pNgsiEntityType,lPropId,'has property');
      }
   }
}
