 ...
 *
 */
const CLUSTER_RADIUS	= undefined;	// Optimal radius
...
function serviceWebhdfsAccess(req,res,next)
{
   ...
   if(lFiwareService!=undefined && lFiwareService=='*')
   {
      debug('Wildcards are not allowed. Do your requests individually :: Hadoop User='+lHadoopUser);
      sendError(res,404,'Wildcards are not allowed. Do your requests individually');
   }
   else if(lFiwareService!=undefined && lEntityType!=undefined && configsys.isHiddenEntity(lHadoopUser,lFiwareService,lEntityType))
   {
   ...
   {
      debug('Your account does not have permissions for using WebHDFS :: Hadoop User='+lHadoopUser);
      sendError(res,403,'Your account does not have permissions for using WebHDFS');
   }
...
function loadSaveSendSchema(res,pFiwareService,pEntityType,pCacheFile,pFormat,next)
{
      ...
      if(lSchemaAsTableAndSchema!=undefined && lSchemaAsTableAndSchema.getRows().length>0)
...
function loadSaveSendEntityRelationships(res,pService,pEntityType,pCacheFile,pFormat,next)
{
      ...
      metadatasys.toTableEntityRelationships(pService,pEntityType!=undefined?[pEntityType]:null).then((lSchemaAsTableAndSchema) => {
                                                      ...
                                                      {
                                                         sendError(res,404,'Error loading relationships information about Entities');
                                                      }
                                              })
                                              .catch((error) => {
                                                      debug(error)
...
function loadSaveSendCurrentNgsiV2(res,pFiwareService,pEntityType,pCacheFile,pFormat,next)
{
      ...
      debug('loadSaveSendCurrentNgsiV2('+pFiwareService+','+pEntityType+','+pCacheFile+','+pFormat+')');
      var lQuery={};
      var lObjQuery = utils.getObjectQuery(lQuery,pEntityType,lEntityId,undefined,false);
      debug('Query (V2): '+JSON.stringify(lObjQuery));
      var lEndpoint=configsys.getOrionEndpoint(lFiwareService);
      debug('Endpoint: '+lEndpoint);

      ngsiv2.listEntities(lEndpoint,lObjQuery,pFiwareService,lFiwareServicePath,lExtended,pEntityType,9999)
          .then((tableAndSchema) => {
                              //debug(JSON.stringify(tableAndSchema,null,2));
...
function loadSaveSendGeoMetadataNgsiV2(res,pFiwareService,pFiwareServicePath,pEntityType,pCacheFile,pFormat,next)
{
      ...
      ngsiv2.listGeoMetadataEntities(pFiwareService,pFiwareServicePath,pEntityType,CLUSTER_RADIUS)
          .then((tableAndSchema) => {
...
{
      ngsildv1.listGeoMetadataEntities(pFiwareService,pFiwareServicePath,pEntityType,CLUSTER_RADIUS)
          .then((tableAndSchema) => {
                              //debug('GeoMetadataNgsiLD: \n'+JSON.stringify(tableAndSchema,null,2));
                              saveDatasetInCache(res,tableAndSchema,pFiwareService,pEntityType,pCacheFile,pFormat,next);
                           })
          .catch((err) => {
                              debug('Error loading GeoMetadataNgsiLD: '+JSON.stringify(err));
                              sendError(res,500,err.name);
                          })
}
...
{
      var lTableValues=pTableAndSchema.getRows();
      var lTableSchema=pTableAndSchema.getSchema();
      ...
      else
      {
           ...
           //debug('Table: '+JSON.stringify(lTableValues,null,2));
           //debug('Schema: '+JSON.stringify(lTableSchema,null,2));
...
function doListStatusServiceEntityJoins(req,res,next)
{
    ...
    else
    {
       ...
       {
           debug('RelationShipField: '+lField);
           ...
           else
           {
               ngsildv1.createPromiseListTypes(lFiwareService)
                             ...
                             .catch((err) => {
                                                ...
                                                sendError(res,500,'Error getting types. Broker response: '+err.name);
                                             });
           }
     }
     else if(configsys.isVersionV2(lFiwareService))
...
function createPromiseCurrentNgsiLD(pFiwareService,pEntityType,pJoinAttrib,pJoinEntityType)
{
    ...
    // Get the Query
    var lObjQuery = utils.getObjectQuery(lQuery,pEntityType,lEntityId,true);
    debug('Query: '+JSON.stringify(lObjQuery));
    return ngsildv1.listEntities(lEndpoint,lObjQuery,pFiwareService,lFiwareServicePath,lExtended,pJoinEntityType,pJoinAttrib,99999);
}
