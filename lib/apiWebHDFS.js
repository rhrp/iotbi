...
function serviceWebhdfsDebug(req,res,next)
{
   ...
   debug('URL:'+req.url);
   debug('UrlPath:'+req.path);
   debug('Path: '+getPath(req)+'   '+PARAM_OP+'='+lOperation);
   debug('Params: '+JSON.stringify(req.params));
   debug('Query: '+JSON.stringify(req.query));
   debug('Hadoop User: '+getQueryParamHadoopUser(req));
   next();
   debug('Request Counter::Waiting for WebHDFS...');
}
/**
...
function serviceWebhdfsAccess(req,res,next)
{
   ...
   else if(lFiwareService!=undefined && !configsys.allowScopeToUser(lHadoopUser,lFiwareService))
   {
      ...
      debug('Your account does not have access to this fiware service :: Hadoop User='+lHadoopUser+' FiwareService='+lFiwareService);
      sendError(res,403,'Your account does not have access to this fiware service');
   }
   else
   {
      next();
...
function loadSaveSendCurrentNgsiV2(res,pFiwareService,pEntityType,pCacheFile,pFormat,next)
{
      ...
      var lExtended=false;
      ngsiv2.createPromiseListEntities(pFiwareService,lFiwareServicePath,lExtended,pEntityType)
          .then((result) => {
                              var lTableAndSchema=ngsiv2.entitiesToTable(result.results,lExtended);
                              saveDatasetInCache(res,lTableAndSchema,pFiwareService,pEntityType,pCacheFile,pFormat,next);
                           })
          .catch((err) => {
...
function doListStatusEntity(req,res,next)
{
    ...
    else if(configsys.isVersionLDV1(lFiwareService))
    {
       ...
       .then(() =>  ngsildv1.createPromiseGetType(lFiwareService,lEntityType)
                            .then((type) => {
                                               res.status(200).json(webhdfsobjs.toFileStatuses(toEntityTypeToFiles(lEntityType),BLOCK_SIZE));
                                            })
                            .catch((err) => {
 ...
 */
function toEntityTypeToFiles(pEntityType)
{
