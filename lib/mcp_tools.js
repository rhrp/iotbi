...
const HARD_LIMIT_TOPK_CELLS = 10000;
/**
...
/**
 * Schema of full-text searches 
 */
const zNgsiQueryFullTextParameter = z.object({
    text:z.string().optional().describe('Text to be searched in text atributes'),
    attribute:z.string().optional().describe('Attribute to be used in the search. Leave in blank to search in all fields')
   });

/**
 * Schema of ger query parameter 
 */
const zNgsiGeoQueryParameter = z.object({
     georel:z.string().describe('The geospatial relationship. Examples: `near;maxDistance==10000` or `near;minDistance==10000` or `within` using meter as unit of distance'),
     geometry:z.string().describe('The type of geometry. Possible value:  `Point` or `Polygon`'),
     coordinates:z.string().describe('The coordinates using square brackets. In case of Point [longitude, latitude] and Polygon [[[longitude, latitude],[longitude, latitude],..]].'),
     geoproperty:z.string().describe('The name of property to be queried. e.g.: `location`'),
   });
...
exports.getGeoMetadataMetadata =  {
        ...
        outputSchema: {
                       geoMetadataEntities:z.array(zNgsiEntityGeoCluster).optional().describe('List of geo clusters'),
...
exports.getGraphEntitiesMetadata =  {
        ...
        inputSchema: {entityType:z.string().describe('NGSI Entity')},
        outputSchema: {
                         graphOfEntities:graphmodel.getSchemaZ().optional().describe('Graph of the NGSI entity describing their structure and attributes'),
                         error:z.string().optional().describe('Error description')
                      }
    };
...
exports.execQueryNGSIMetadata =  {
        ...
        description: 'Retrieves data from the NGSI broker filtred by geospatial parameters, NGSI query in numerical properties and full-text searches on text properties',
        inputSchema: {entityType:z.string().describe('An entityType provided by the `getAvailableEntities` tool'),
                      attribList:z.string().optional().describe('Comma separated list of attribute values, provided by the `getGraphEntities` tool'),
                      geoQuery:zNgsiGeoQueryParameter.optional().describe('Geoquery for spatial filtering adjusted to the user\' question.'),
                      ...
                      fullTextSearch:zNgsiQueryFullTextParameter.optional().describe('A string to be used in a full-text search on a list of attributes.'),
                      entityId:z.string().optional().describe('The Id of the entity to search'), 
                      },
        outputSchema: {
                        result:zNgsiQueryResult.optional(),
                        error:z.string().optional().describe('Error description')
                      }
    };
/**
...
{
    var lTopK=HARD_LIMIT_TOPK_CELLS;
    var lEntityType=pParameters.entityType;
    var lAttribList=pParameters.attribList;
    var lGeoQuery=pParameters.geoQuery;
    var lQuery=pParameters.queryNGSI;
    var lFullTextSearch=pParameters.fullTextSearch;
    var lEntityId=pParameters.entityId;
    var lQueryDetails='execQueryNGSI('+lEntityType+',<'+lAttribList+'>,<'+JSON.stringify(lGeoQuery)+',<'+lQuery+'>,<'+JSON.stringify(lFullTextSearch)+'>,'+lEntityId+')';
    ...
    if(lFullTextSearch!=undefined && lFullTextSearch.text!=undefined && lFullTextSearch.text.length>0)
    {
       if(lFullTextSearch.text=='*' || lFullTextSearch.text=='%')
       {
           debug('Wildcard');
       }
       else
       {
           let lTextQueryValidation=lBrokerParallel.setTextPattern(lFullTextSearch.text,lFullTextSearch.attribute);
           debug('setTextPattern('+lFullTextSearch.text+','+lFullTextSearch.attribute+') Validation='+lTextQueryValidation);
       }
    }
    if(lAttribList==undefined)
    {
       debug('Empty attribute lists');
    }
    //Limit of rows in the result
    lBrokerParallel.setTopK(lTopK);
    if(lGeoQuery!=undefined)
    {
        var lGeorel=lGeoQuery.georel;
        var lGeometry=lGeoQuery.geometry;
        var lCoordinates=lGeoQuery.coordinates;
        var lGeoproperty=lGeoQuery.geoproperty;
    ...
    await lBrokerParallel.getCurrentData(lEntityType,lAttribList,lEntityId,undefined,undefined,undefined,20)
        .then((lSortedResult) => {
           let lTable=lSortedResult.getTable(true);
           debug(lQueryDetails);
...
exports.ngsiQuery=async function(uri,entityType,attribList,b64georel,b64geometry,b64coordinates,geoproperty,b64query,b64textPattern,entityId)
{
  ...
  lBrokerParallel.setTextPattern(lTextPattern,undefined);

  await lBrokerParallel.getCurrentData(lEntity,lAttribs,lEntityId,undefined,undefined,undefined,20)
        .then((lSortedResult) => {
