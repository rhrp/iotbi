...
//const HARD_LIMIT_TO_RANK = 27000;
const HARD_LIMIT_ROWS_NOSCORED = 20;
...
module.exports = class BrokerParallelize {
   ...
   */
   constructor(fiwareServices) 
   {
      ...
      this.topK=undefined;
      this.noScoredRows=HARD_LIMIT_ROWS_NOSCORED;
      this.requestId=0;
   }
   ...
   setNoScoredRows(noScoredRows)
   {
      this.noScoredRows=noScoredRows;
   }
   ...
   */
   ...
   #__getCurrentData(pEntityType,pAttribs,pEntityId,pExtended,pTableNameJoin,pJoinAttribute,pMaxColumns)
   {
      ...
      var lTopK=this.topK;
      let lNoScoredRows=this.noScoredRows;
         ...
         Promise.all(lPromises)
            .then(results => {
               ...
               if(lTableAcumulator.countRows()==0)
               {
                   ...
                   let lUnsortedResult=lRank.unsorted(undefined,'Empty result');
...
/**
               else if(lTextPattern!=undefined && (lTableAcumulator.countRows()*lTableAcumulator.countCols()>HARD_LIMIT_TO_RANK))
               {
                      ...
                      //lUnsortedResult.debugResult();
                      //resolve(lUnsortedResult);
                      reject('The result is too long. Apply geospatial filtering based on geographic coordinates using the parameter geoquery or reduce the number of columns.');
               }
*/
               else if(lTextPattern!=undefined && lTableAcumulator.countRows()>0)
               {
                      ...
                      lRank.sortByRelevance(lTopKRows,undefined,lNoScoredRows)
               ...
               else
               {
                      ...
                      let lTopKRows=BrokerParallelize.calcTopKRows(lTableAcumulator,lTopK);
                      PrintTable.printTable(lTableAcumulator,debug);
