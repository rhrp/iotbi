...
module.exports = class BrokerParallelize {
   ...
   */
   ...
   #__getCurrentData(pEntityType,pAttribs,pEntityId,pExtended,pTableNameJoin,pJoinAttribute)
   {
      ...
      let lRequestId=this.requestId;
      perfanalyser.mark(lRequestId,perfanalyser.OP_GETCD,'startInvoke');
      return new Promise(function(resolve, reject) {
         Promise.all(lPromises)
            .then(results => {
               lSucessCall=true;
               perfanalyser.mark(lRequestId,perfanalyser.OP_GETCD,'startMerge');
          ...
          .finally(() => {
             if(lSucessCall)
             {
                perfanalyser.mark(lRequestId,perfanalyser.OP_GETCD,'endMerge');
             }
           });
       });
   }
