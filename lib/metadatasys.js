...
function toTableEntitySchema(pNgsiEntityType)
{
   ...
   if(lEntitySchema==undefined)
   {
      ...
      return new TableModel('EntitySchema-'+pNgsiEntityType,lTable,lSchema);
   }
...
function toTableListOfEntities(pFiwareService)
{
   ...
   return new TableModel('ListEntities-'+pFiwareService,lTable,lSchema);
}
...
function toTableEntityRelationships(pServices,pListEntityTypes)
{
   return new Promise(function(resolve, reject) {
      ...
      {
        if(!configsys.isVersionLDV1(lService))
        {
           debug(`The service ${lService} is not NGSI-LD-based (${configsys.getBrokerNGSIVersion(lService)})`);
        }
        else
        { 
           for(lEntityType in configsys.getNgsiLdEntityAllContexts())
      ...
      Promise.all(lPromises)
            .then(results => {
               ...
               resolve(new TableModel('EntityRelationships',lTable,lSchema));

       })
      .catch(error => {
          debug(`Error toTableEntityRelationships('${pServices}','${pListEntityTypes}')`)
          debug(error);
          reject(error);
      });
  });
