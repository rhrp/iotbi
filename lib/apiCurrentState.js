...
exports.service = function(req,res,next)
{
   ...
   // Fulltext-search
   let lTextPattern=lQuery.textPattern;
   let lSearchField=lQuery.searchField;
   let lTopk=lQuery.topk || undefined;
   // Format
   var lFormat=utils.getParamFormat(req); 
   let lAttribs=lQuery.attrs;
   ...
   debug(`EntityType: ${lEntityType}   EntityId: ${lEntityId}  Full-text search: ${lTextPattern} at ${lSearchField}`);
   lBrokerParallel.setTextPattern(lTextPattern,lSearchField);
   if(lTextPattern!=undefined & lSearchField!=undefined)
   {
     lBrokerParallel.setTopK(lTopk);
     lBrokerParallel.setNoScoredRows(0);
   }
   //let lValidQuery=lBrokerParallel.setQuery(lQuery,lEntityType);
   lBrokerParallel.getCurrentData(lEntityType,lAttribs,lEntityId,lExtended,lJoinTable,undefined)
        .then((lSortedResult) => {
