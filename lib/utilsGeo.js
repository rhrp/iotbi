...
function toTableGetMetadata(pFiwareService,pEntityType,pRadius,pPoints)
{
     ...
     if(checkListOfPoints(pPoints))
     {
       ...
       for(clusterId=0;clusterId<lCentroids.length;clusterId++ )
       {
          ...
          if(lClusterPoints.length>0)
          {
            ...
            lTable.push(lRow);
            lCountClusteredPoints=lCountClusteredPoints+lClusterPoints.length;
          }
          else
          {
     ...
     debug('Total of Points: '+(pPoints!=undefined?pPoints.length:undefined)+'   Total of Points in clusters: '+lCountClusteredPoints+'   Clusters:'+(lCentroids!=undefined?lCentroids.length:undefined));
