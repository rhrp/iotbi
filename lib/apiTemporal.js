...
function findSchema(pTablePlusSchema,pDictTableSchema)
{
    ...
    if(lTable.length==0)
    {
       ...
       return new TableModel('table-find-schema',pTablePlusSchema[0],pTablePlusSchema[1]);
    }
 ...
 */
function mergeEntitiesToTemporalQL(res,next,pTemporalTable,pEntityType,pFiwareService,pFiwareServicePath,pFormat,pExtended,pAddEntityProps)
{
	...
	debug('Type:'+pEntityType);
        ...
        if(configsys.isOrionLD(pFiwareService))
        {
          ...
          .then((result) => {
                              utils.saveOutputTable(res,merge(pTemporalTable,result.results,mergeEntityLD,ngsildv1.entityToRow,pExtended,pAddEntityProps),pFormat,pEntityType);
                              next();
                            })
          .catch((err) => {
                              utils.saveOutputError(res,500,err,pEntityType);
                              next();
                          })
        }
        else if(configsys.isOrionV2(pFiwareService) && USE_NEW_LIB)
        {
          ...
          .then((result) => {
                              utils.saveOutputTable(res,merge(pTemporalTable,result.results,mergeEntityV2,ngsiv2.entityToRow,pExtended,pAddEntityProps),pFormat,pEntityType);
                              next();
                          })
          .catch((err) => {
                              utils.saveOutputError(res,500,err,pEntityType);
	  ...
	  .then((result) => {
                              utils.saveOutputTable(res,merge(pTemporalTable,result.body,mergeEntityV2,ngsiv2.entityToRow,pExtended,pAddEntityProps),pFormat,pEntityType);
                              next();
                            })
	  .catch((err) => {
                            utils.saveOutputError(res,500,err,pEntityType);
        ...
        {
            utils.saveOutputError(res,500,'Invalid Orion version config',pEntityType);
            next();
        }
        debug('Wait for Broker response...');
}
 ...
 */
function invokeRequestQuantumLeap(req,res,next,pUrl,lEntityType,lFiwareService,lFiwareServicePath,lFormat,pExtended,pOffset,pPageSize,pLimit,pTablePlusSchema)
{
   ...
   invokePartQuantumLeap(req,res,lUrl,lEntityType,lFiwareService,lFiwareServicePath,lFormat,pExtended,pOffset,pPageSize).then(function(result) {
        ...
        if(pTablePlusSchema[0].length>pLimit)
        {
           ...
           utils.saveOutputError(res,500,'Limit exceded: '+pLimit,lEntityType);
           next();
        }
        else if(lRowIndex<pPageSize)
        {
           mergeEntitiesToTemporalQL(res,next,pTablePlusSchema,lEntityType,lFiwareService,lFiwareServicePath,lFormat,pExtended,pExtended);
        }
        else
        {
           //Load next page
           invokeRequestQuantumLeap(req,res,next,pUrl,lEntityType,lFiwareService,lFiwareServicePath,lFormat,pExtended,pOffset+pPageSize,pPageSize,pLimit,pTablePlusSchema);
        }
     })
    .catch(function(err) {
    ...
    debug('After invokeRequestQuantumLeap...');
}
 ...
 */ 
function invokeRequestTemporalAPI(req,res,next,pConnection,pOptions,pEntityType,pFiwareService,pFiwareServicePath,pFormat,pExtended,pOffset,pPageSize,pLimit,pTable,pJoinTableName)
{
   var pEntityTypes=[];
   ...
   ngsildcontext.createPromisesloadNgsiContext(utils.toArray(pFiwareService,undefined),lEntitiesToLoad)
   .then(function (lNgsiContextData) {
      ...
      pConnection.ld.queryTemporalEntities(pOptions)
		 ...
		 debug('TemporalResult:\n'+JSON.stringify(resultTemporal,null,2));
                 ...
                 mergeEntitiesToTemporalNGSI(req,res,next,lTable,pFiwareService,pFiwareServicePath,pEntityType,pJoinTableName,pExtended,pFormat);
            }).catch(function(err) {
   ...
   debug('After invokeRequestTemporalAPI'); 
}
 ...
 */
function mergeEntitiesToTemporalNGSI(req,res,next,pTemporalTable,pFiwareService,pFiwareServicePath,pEntityType,pJoinTableName,pExtended,pFormat)
{
     ngsildv1.createPromiseQueryEntities(pFiwareService,pFiwareServicePath,pEntityType)
     .then((resultEntities) => {
                           ...
                           if(pJoinTableName==undefined)
                           {
                              utils.saveOutputTable(res,lMergedTable,pFormat,pEntityType);
                           }
                           else
                           {
     ...
     debug('After mergeEntitiesToTemporalNGSI...');
}
/**
 * Join a table
 */
function loadAndJoinTable(req,res,pMasterTable,pFiwareService,pFiwareServicePath,pMainEntityType,pJoinTableName,pExtended,pFormat)
{
   ...
   ngsildv1.createPromiseQueryEntities(pFiwareService,pFiwareServicePath,pJoinTableName)
          .then((resultEntities) => {
                             ...
                             utils.saveOutputTable(res,lJoinedTable,pFormat,pMainEntityType)
                           })
          .catch((err) => utils.saveOutputError(res,500,err,pEntityType));
   debug('After loadAndJoinTable...');
}
exports.service = function(req,res,next)
{
   ...
   var lCoords=lQuery.coordinates;
   ...
   if(lLimit>HARD_LIMIT)
   {
        ...
        utils.saveOutputError(res,413,lError,pEntityType);
        debug(lError);
        next();
   }
   else if(configsys.isBrokerSuportsTemporalAPI(lFiwareService))
   {
        debug('Using NGSI-LD Temporal API');
        serviceNGSITemporalAPI(req,res,next,lFiwareService,lFiwareServicePath,lEntityType,lEntityId,lAttrs,lIdPattern,lFromDate,lToDate,lCoords,lMinDistance,lMaxDistance,lGeorel,lFormat,lExtended,lLimit,lJoinTable);
   }
   else if(configsys.getQuantumLeapServerOk(lFiwareService))
   {
        debug('Using NGSI-TSDB API');
        serviceQuantumLeap(req,res,next,lFiwareService,lFiwareServicePath,lEntityType,lEntityId,lAttrs,lIdPattern,lFromDate,lToDate,lCoords,lMinDistance,lMaxDistance,lGeorel,lFormat,lExtended,lLimit);
   }
   else
   {
        var lError='Invalid Service '+lFiwareService;
        //res.status(404).json({'description':lError});
        utils.saveOutputError(res,404,lError,pEntityType);
        next();
   }
   debug('After service...');
}

function serviceQuantumLeap(req,res,next,pFiwareService,pFiwareServicePath,pEntityType,pEntityId,pAttrs,pIdPattern,pFromDate,pToDate,pCoords,pMinDistance,pMaxDistance,pGeorel,pFormat,pExtended,pLimit)
{
   ...
   lUrl=utils.addGeoLocation(lUrl,null,pCoords,pMinDistance,pMaxDistance,pGeorel,false);

   invokeRequestQuantumLeap(req,res,next,lUrl,pEntityType,pFiwareService,pFiwareServicePath,pFormat,pExtended,0,QL_LIMIT,pLimit,[[],{}])
   debug('QuantumLeap Service Ok!');
}
function serviceNGSITemporalAPI(req,res,next,pFiwareService,pFiwareServicePath,pEntityType,pEntityId,pAttrs,pIdPattern,pFromDate,pToDate,pCoords,pMinDistance,pMaxDistance,pGeorel,pFormat,pExtended,pLimit,pJoinTable)
{
   debug('Start serving serviceNGSITemporalAPI ('+pFiwareService+')...');
   ...
   var lTable=[];
   invokeRequestTemporalAPI(req,res,next,lConnection,lOptions,pEntityType,pFiwareService,pFiwareServicePath,pFormat,pExtended,0,TA_LIMIT,pLimit,lTable,pJoinTable)
